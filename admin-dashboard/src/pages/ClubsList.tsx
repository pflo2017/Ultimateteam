...
const ClubsList: React.FC = () => {
  ...
  const fetchClubs = async () => {
    ...
    try {
      ...
      // Log the first club's structure to see what fields are available
      if (clubsData && clubsData.length > 0) {
        console.log('First club structure:', Object.keys(clubsData[0]));
        console.log('First club full data:', JSON.stringify(clubsData[0], null, 2));
        console.log('First club admin fields:', {
          admin_name: clubsData[0].admin_name,
          admin_email: clubsData[0].admin_email,
          admin_id: clubsData[0].admin_id
        });
        
        // Log all clubs' admin data
        console.log('All clubs admin data:', clubsData.map(club => ({
          name: club.name,
              ...
              {filteredClubs.length > 0 ? (
                filteredClubs.map((club) => (
                  <tr key={club.id}>
                    ...
                    <td>
                      <Text size="sm" color="dimmed">{club.admin_email || club.email || '-'}</Text>
                    </td>
                    <td>
