...
-- Allow coaches to record attendance for their team's activities - using auth.uid() since recorded_by references auth.users(id)
...
WITH CHECK (
  EXISTS (
    ...
    WHERE a.id::text = activity_attendance.activity_id::text
