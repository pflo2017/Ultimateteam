-- Fix security warnings in Supabase database using transactions for safety
...
SELECT c.*,
       -- Replace auth.users references with NULL or fields from clubs table
...
-- 2. Copy the view body (everything between AS and WITH) to maintain functionality
-- 3. Only change the SECURITY DEFINER to SECURITY INVOKER and fix auth.users exposure 