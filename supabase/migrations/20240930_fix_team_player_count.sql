...
-- Create updated function to get coach's teams that only counts active players
...
BEGIN
    ...
    FROM teams t
    LEFT JOIN players p ON p.team_id = t.id AND p.is_active = true  -- Only count active players
